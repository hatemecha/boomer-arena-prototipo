...
func _ready() -> void:
	...
	if map_model == null:
		push_error("Dust2Arena requires a valid MapModel node.")
		return
	...
	if collision_mesh_count == 0:
		push_error("Dust2Arena could not create collision because the model contains no meshes.")
...
func _on_void_recovery_body_entered(body: Node3D) -> void:
	...
	if game_root == null or not game_root.has_method("recover_player_from_world_bounds"):
		push_error("Dust2Arena requires a game root that can recover out-of-bounds players.")
		return
