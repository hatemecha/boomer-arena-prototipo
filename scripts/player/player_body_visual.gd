...
func update_third_person_visual(player, delta: float) -> void:
	...
	var speed_ratio: float = clampf(horizontal_speed / maxf(player.run_speed, 0.001), 0.0, 1.0)
	...
	var strafe_ratio: float = clampf(local_velocity.x / maxf(player.run_speed, 0.001), -1.0, 1.0)
	var forward_ratio: float = clampf(-local_velocity.z / maxf(player.run_speed, 0.001), -1.0, 1.0)
	...
	var pitch_radians: float = deg_to_rad(clampf(player.camera_pivot.rotation_degrees.x, -55.0, 55.0))
...
func _cache_limb_bones(player) -> void:
	...
	for limb_name in bone_names.keys():
		var bone_index: int = player.character_skeleton.find_bone(String(bone_names[limb_name]))
		if bone_index < 0:
			continue
...
func _update_limb_animation(player, speed_ratio: float, walk_wave: float) -> void:
	...
	var jump_tuck: float = clampf(absf(player.velocity.y) / maxf(player.jump_velocity, 0.001), 0.0, 1.0)
	...
	_apply_limb_pose(player.character_skeleton, "left_upper", Vector3.RIGHT, left_angle)
	_apply_limb_pose(player.character_skeleton, "right_upper", Vector3.RIGHT, right_angle)
	_apply_limb_pose(player.character_skeleton, "left_lower", Vector3.RIGHT, -left_angle * 0.45 - tuck * 0.25)
	_apply_limb_pose(player.character_skeleton, "right_lower", Vector3.RIGHT, -right_angle * 0.45 - tuck * 0.25)
	_apply_limb_pose(player.character_skeleton, "left_foot", Vector3.RIGHT, -left_angle * 0.25)
	_apply_limb_pose(player.character_skeleton, "right_foot", Vector3.RIGHT, -right_angle * 0.25)
